    ...
    public partial class MainView : BaseForm, Ultra.Surface.Interfaces.ISurfacePermission
    {
        ...
        public List<Ultra.Surface.Interfaces.PermitGridView> Grids {
            get {
                ...
                new Ultra.Surface.Interfaces.PermitGridView(this.gridView1,"上报积分")
            };
            }
        }
        ...
        void btnInvalid_ItemClick(object sender, ItemClickEventArgs e) {
            ...
            using (var db = new Database()) {
                db.Update<t_approvepoint>(" set IsInvalid=1 where guid=@0", et.Guid);
            }
            barBtnRefresh_ItemClick(null, null);
        }

        void btnSubmit_ItemClick(object sender, ItemClickEventArgs e) {
            var et = gridControlEx1.GetFocusedDataSource<t_approvepoint>();
            if (null == et) return;
            using (var db = new Database()) {
                try {
                    db.BeginTransaction();

                    db.Update<t_member>(" set CurPointFee=CurPointFee-@0 where receivername=@1", et.PointFee, et.FromMember);

                    var mem = db.FirstOrDefault<t_member>(" where receivername=@0", et.FromMember);

                    db.Save(new t_pointfeelog() {
                        RecvName = et.FromMember,
                        Desc = "上报积分至" + et.ToMember + "-" + et.ToMemberNo,
                        PointFee = et.PointFee,
                        CurPointFee = mem.CurPointFee,
                        RecvPointFee = mem.RecvPointFee,
                    ...
                    db.Update<t_approvepoint>(" set IsSubmit=1 where guid=@0", et.Guid);
                    db.CompleteTransaction();

                    MsgBox.ShowMessage("提示","积分扣除成功!");
                } catch (Exception) {
                    db.AbortTransaction();
                    throw;
                }
        ...
        void barBtnRefresh_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            switch (tabMain.SelectedTabPage.Text) {
                case "未提交":
                    using (var db = new Database()) {
                        this.gridControlEx1.DataSource = db.Fetch<t_approvepoint>("where isnull(issubmit,0)=0 and isnull(isinvalid,0)=0");
                    }
                    break;
                case "已提交":
                    using (var db = new Database()) {
                        this.gridControlEx2.DataSource = db.Fetch<t_approvepoint>(" where isnull(issubmit,0)=1 and isnull(isinvalid,0)=0");
                    }
                    break;
                default:
                    break;
